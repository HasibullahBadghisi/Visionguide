...
const LiveSession: React.FC<Props> = ({ mode, onExit }) => {
  ...
  useEffect(() => {
    const handleOffline = () => {
      ...
      speakStatus("Internet connection lost. I cannot see. Stopping.");
      setIsConnected(false);
      serviceRef.current?.stopAllAudio();
      // Optionally disconnect fully to force reconnection logic when online
      serviceRef.current?.disconnect();
    };

    const handleOnline = () => {
      // We don't auto-reconnect to avoid state confusion, just prompt user
      setError("Internet Restored. Please restart session.");
      speakStatus("Internet restored. Please restart the session.");
    };
  ...
  const initSession = useCallback(async () => {
    if (!navigator.onLine) {
       setError("No Internet Connection");
       speakStatus("No internet connection. I need the internet to see.");
       return;
    }
